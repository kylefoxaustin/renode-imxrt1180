...
| **Zephyr console delta** | **75 / 75 scoreable targets agree**, over 90 targets — 72 measured (46 byte-identical + 26 differing only in durations/counters) plus 3 adjudicated, with **0 unexplained content differences**. Each adjudication is signed off in [`results/zephyr-delta-adjudications.tsv`](results/zephyr-delta-adjudications.tsv) and bound to a hash of its exact diff, so a regression cannot inherit the excuse. Was 70/76 raw (73/76 adjudicated) before the unmapped-access fix. ⚠ The scoreable denominator moved 76 → 75: `arm_mpu_wt` left it by becoming **byte-identical** to QEMU (both now bus-fault at `0x30500000` and halt the same way, so neither prints a completion marker and the classifier will not score a run with no verdict) — it was 50 lines vs QEMU's 24 before. 15 targets are **unscoreable** — free-running samples that never terminate, whose line count measures the run guard, not the model; they are counted neither way |
