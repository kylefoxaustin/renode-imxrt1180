...
namespace Antmicro.Renode.Peripherals.I2C
{
    ...
    public class IMXRT1180_LPI2C : SimpleContainer<II2CPeripheral>, IDoubleWordPeripheral, IProvidesRegisterCollection<DoubleWordRegisterCollection>, IKnownSize, INumberedGPIOOutput
    {
        public IMXRT1180_LPI2C(IMachine machine) : base(machine)
        {
            IRQ = new GPIO();
        ...
        // ── DMA REQUEST LINES ────────────────────────────────────────────────────
        //
        // GPIO 0 = TX request (eDMA3 source 7 for LPI2C1), GPIO 1 = RX request
        // (source 8). Wired in the .repl through IMXRT1180_DMARequestRouter, which
        // translates a SOURCE number into the channel whose CH_MUX selected it.
        //
        // ⭐ THE LEVEL IS HELD, NOT PULSED. @rt1180emulator's model drives these as
        // booleans re-evaluated on every relevant state change, and the eDMA sees a
        // held level for as long as the condition holds. Renode's eDMA services ONE
        // minor loop per rising edge, so a held level is expressed by RE-ARMING after
        // each service -- and the re-arm is driven by the eDMA's OWN write to MTDR.
        // That makes it self-limiting: when the channel's major loop is exhausted it
        // stops writing MTDR, so the requests stop. The same "a minor loop lowers its
        // own request" handshake the SAI uses, and the reason it cannot spin.
        public IReadOnlyDictionary<int, IGPIO> Connections => dmaConnections;

        private bool TransmitRequestActive => controllerEnabled.Value && transmitDmaEnabled.Value;

        private bool ReceiveRequestActive => controllerEnabled.Value && receiveDmaEnabled.Value
                                             && rxQueue.Count > (int)rxWatermark.Value;

        private void UpdateDmaRequests()
        {
            dmaConnections[TransmitRequestLine].Set(TransmitRequestActive);
            dmaConnections[ReceiveRequestLine].Set(ReceiveRequestActive);
        }

        // The eDMA's own read of MRDR is the RX handshake, mirroring the MTDR write
        // on the transmit side: drain a byte, drop the level, and re-assert only if
        // the FIFO still satisfies the watermark. Self-limiting for the same reason.
        private void RearmReceiveRequest()
        {
            var line = dmaConnections[ReceiveRequestLine];
            line.Set(false);
        ...
        {
            if(!TransmitRequestActive)
            {
                return;
            }
            var line = dmaConnections[TransmitRequestLine];
            line.Set(false);
            machine.LocalTimeSource.ExecuteInNearestSyncedState(_ =>
            {
                if(TransmitRequestActive)
        ...
        private readonly Dictionary<int, IGPIO> dmaConnections = new Dictionary<int, IGPIO>
        {
        ...
        private void HandleCommand(Command command)
        {
            ...
            case Command.ReceiveDataAndDiscard:
                ...
                if(command != Command.ReceiveDataAndDiscard)
                {
                    ...
                    // ⭐ THE RX REQUEST LEVEL DEPENDS ON THE FIFO COUNT, SO IT HAS TO BE
                    // RE-EVALUATED WHERE THE COUNT CHANGES. Updating it only on MEN/MDER
                    // writes meant bytes could arrive with the line still low and the RX
                    // channel never got a service request -- the guest then spun on
                    // CH_CSR[DONE] forever. A level is only "held" if something holds it.
                    UpdateDmaRequests();
                    UpdateInterrupts();
                }
                break;
        ...
        private void DefineRegisters()
        {
            ...
            Registers.ControllerControl.Define(this)
                .WithFlag(0, out controllerEnabled, name: "ControllerEnable",
                    changeCallback: (_, value) => { if(value) TryFlushTransmitFIFO(); UpdateDmaRequests(); })
            ...
            // ⭐⭐ MDER.TDDE/RDDE WERE `WithTaggedFlag` -- ACCEPTED AND DISCARDED.
            // That is why imxrt1180-lpi2c-dma reported "TX gate stuck": the request
            // line could not be gated by a bit the model never stored, so the
            // NEGATIVE phases (MDER=0 -> the TX line must stay DOWN; MDER=TDDE only
            // -> RX must stay down) could never pass. A tagged flag is the register
            // form of accepting a write and changing nothing.
            //
            // Gating, from @rt1180emulator's anchors:
            //   TX request = MEN && TDDE            (the TX FIFO is synchronous here,
            //                                        so TDF is always set and the
            //                                        request reduces to the enables)
            //   RX request = MEN && RDDE && (rxCount > RXWATER)
            Registers.ControllerDMAEnable.Define(this)
                .WithFlag(0, out transmitDmaEnabled, name: "TransmitDataDMAEnable")
                .WithFlag(1, out receiveDmaEnabled, name: "ReceiveDataDMAEnable")
                .WithReservedBits(2, 30)
                .WithChangeCallback((_, __) => UpdateDmaRequests())
            ;
            ...
            Registers.ControllerTransmitData.Define(this)
                ...
                .WithEnumField<DoubleWordRegister, Command>(8, 3, name: "CommandData",
                    writeCallback: (_, value) => { HandleCommand(value); RearmTransmitRequest(); })
                .WithReservedBits(11, 21)
            ;

            Registers.ControllerReceiveData.Define(this)
                // ════ [RT1180 PATCH — MRDR.RXEMPTY MUST DESCRIBE THE BYTE BEING RETURNED] ══
                // Upstream computes bit 14 as `rxQueue.Count == 0`, evaluated AFTER the
                // data field's provider has already dequeued. So the LAST byte of every
                // drain comes back as data|RXEMPTY. MEASURED on the stock prebuilt NXP
                // demo_apps/bubble_peripheral reading FXLS8974 WHO_AM_I:
                //
                //   ReadUInt32 from 0x70 (ControllerReceiveData), returned 0x4086
                //                                                          ^^^^ ^^
                //                                             RXEMPTY=1 ---'    '--- 0x86
                //
                // fsl_lpi2c treats RXEMPTY=1 as "no valid data" and discards the byte, so
                // the driver never receives WHO_AM_I and spins: 257751 MRDR reads in 0.05 s
                // of emulated time, all 0x4000, console silent. The QEMU oracle gets this
                // right by testing emptiness BEFORE popping (hw/i2c/imxrt1180_lpi2c.c:295
                // `if (s->rx_count == 0) return MRDR_RXEMPTY;`).
                //
                // RXEMPTY must describe the fetch that produced THIS value, so latch it in
                // the data provider and report the latched flag. (Fields are evaluated
                // low-bit-first, so bit 0's provider runs before bit 14's -- which is the
                // very ordering that causes the upstream bug, and here makes the fix work.)
                // Renode 1.17.0 upstream defect -- see EXPERIMENT.md "defect #6".
                ...
                .WithFlag(14, FieldMode.Read, name: "ReceiveEmpty",
                    valueProviderCallback: _ => lastFetchWasEmpty)
        ...
        private bool lastFetchWasEmpty = true;
